...
const categorySchema = Joi.string().empty().min(3).max(50);

const citySchema = Joi.string().empty().min(3).max(50);

const countrySchema = Joi.string().empty().min(3).max(50);

const coordinatesSchema = Joi.array().items(Joi.number().min(1).required());

const ibuSchema = Joi.number().empty().min(1).required();

const nameSchema = Joi.string().empty().min(3).max(50)
  .required();

const stateSchema = Joi.string().empty().min(3).max(50);
